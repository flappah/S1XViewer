...
{
    public class S104DCF2V11DataParser : HdfDataParserBase, IS104DCF2V11DataParser
    {
        ...
        public S104DCF2V11DataParser(IDatasetReader datasetReader, IGeometryBuilderFactory geometryBuilderFactory, IS111ProductSupport productSupport, IOptionsStorage optionsStorage)
        {
        ...
        public override async Task<IS1xxDataPackage> ParseAsync(string hdf5FileName, DateTime? selectedDateTime)
        {
            ...
            await Task.Run(() =>
            {
                ...
                foreach (Hdf5Element? waterLevelFeatureInstanceNode in featureElement.Children)
                {
                    ...
                    {
                        var gridOriginLatitudeElement = waterLevelFeatureInstanceNode.Attributes.Find("gridOriginLatitude");
                        var gridOriginLatitude = gridOriginLatitudeElement?.Value<double>() ?? -999.0;
                        var gridOriginLongitudeElement = waterLevelFeatureInstanceNode.Attributes.Find("gridOriginLongitude");
                        var gridOriginLongitude = gridOriginLongitudeElement?.Value<double>() ?? -999.0;
                        var gridSpacingLatitudinalElement = waterLevelFeatureInstanceNode.Attributes.Find("gridSpacingLatitudinal");
                        var gridSpacingLatitudinal = gridSpacingLatitudinalElement?.Value<double>() ?? -999.0;
                        var gridSpacingLongitudinalElement = waterLevelFeatureInstanceNode.Attributes.Find("gridSpacingLongitudinal");
                        ...
                        var numPointsLongitudinalNode = waterLevelFeatureInstanceNode.Attributes.Find("numPointsLongitudinal");
                        ...
                        var startSequenceNode = waterLevelFeatureInstanceNode.Attributes.Find("startSequence");
                        ...
                        foreach (Hdf5Element? group in waterLevelFeatureInstanceNode.Children)
                        {
                            ...
                            if (DateTime.TryParseExact(timePointString, "yyyyMMddTHHmmssZ", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime timePoint))
                            {
                                ...
                                var currentDataset =
                                      _datasetReader.ReadArrayOfFloats(hdf5FileName, group.Children[0].Name, numPointsLatitude, numPointsLongitude * 2);
